...
def test_plot_fitness_no_warning(monkeypatch, recwarn):
...
def test_no_optimal_message_on_invalid_fitness(monkeypatch, capsys):
    df = pd.DataFrame(
        {
            'Open': [1, 2],
            'High': [1, 2],
    ...
    def analysis_stub(*a, **k):
        analysis_called['called'] = True

    monkeypatch.setattr(main.analysis, 'run_champion_analysis', analysis_stub)
    ...
    assert 'Optimal Parameters Found' not in captured.out
    assert 'called' not in analysis_called
