...
def test_hard_floor_forbids_coverage_penalty(monkeypatch):
    ...
    stats_seq = [
        {'sortino': 1.0, 'profit_factor': 1.0, 'max_drawdown': 0.0, 'trades': 1,
         'total_return': 0.0, 'equity_curve': pd.Series(dtype=float)},
    ...
    monkeypatch.setattr(fitness.MultiAssetFitnessEvaluator, '_evaluate_single_asset', fake_eval)
...
def test_zero_trade_penalize_includes_asset(monkeypatch):
    ...
    stats_seq = [
        {'sortino': 1.0, 'profit_factor': 1.0, 'max_drawdown': 0.0, 'trades': 1,
         'total_return': 0.0, 'equity_curve': pd.Series(dtype=float)},
        {'sortino': 0.0, 'profit_factor': 1.0, 'max_drawdown': 0.0, 'trades': 0,
         'total_return': 0.0, 'equity_curve': pd.Series(dtype=float)},
    ]
